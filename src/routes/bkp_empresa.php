...
$app->get('/empresas/consulta', function (Request $request, Response $response, array $args) {  
  ...
  $sql = " Select ec_empresa.*, ec_imagem.arquivo, string_agg(CONCAT(cat.nome, ','), '') as categorias from ec_empresa 
    ...
    where 
      ec_empresa.status = 0
      and (unaccent(ec_edificiocomercial.url) = unaccent('". trim(pg_escape_string($dados["texto"])) ."')      
      or unaccent(ec_empresa.nome) ilike unaccent('%". trim(pg_escape_string($dados["texto"])) ."%')
      or unaccent(ec_empresa.descricao) ilike unaccent('%". trim(pg_escape_string($dados["texto"])) ."%')
      or unaccent(ec_empresa.tags) ilike unaccent('%". trim(pg_escape_string($dados["texto"])) ."%'))
...
$app->post('/empresa', function (Request $request, Response $response, array $args) {  
  ...
  try {
    ...
    twitter, tags, status, id_usuario, id_edificiocomercial) VALUES 
    ('".$dados["nome"]."', 
