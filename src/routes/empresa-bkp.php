...
$app->get('/empresas/consulta', function (Request $request, Response $response, array $args) {  
  ...
  $sql = " Select ec_empresa.*, ec_imagem.arquivo, string_agg(CONCAT(cat.nome, ','), '') as categorias from ec_empresa 
    ...
    where 
        ec_edificiocomercial.url = '". $dados["url"] ."'      
		ec_empresa.nome ilike '%". trim(pg_escape_string($dados["texto"])) ."%'
		or ec_empresa.descricao ilike '%". trim(pg_escape_string($dados["texto"])) ."%'
		or ec_empresa.tags ilike '%". trim(pg_escape_string($dados["texto"])) ."%'
