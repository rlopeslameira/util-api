...
$app->get('/empresas/consulta', function (Request $request, Response $response, array $args) {  
  ...
  {  
    if ($dados["categoria"] != ""){
      $where .= " and empcat.id_categoria = ". $dados["categoria"];
    }
  }
  ...
  {  
    if ($dados["status"] != ""){
      $where .= " and ec_empresa.status = ". $dados["status"] ."";
    }
  }else{
    $where .= " and ec_empresa.status = 0 ";
  }
  ...
  $sql = " Select ec_empresa.*, ec_imagem.arquivo, string_agg(CONCAT(cat.nome, ','), '') as categorias from ec_empresa 
    ...
    where ec_empresa.id_empresa > 0 ". $where ."  group by ec_empresa.id_empresa, ec_imagem.arquivo ";
...
$app->post('/empresa', function (Request $request, Response $response, array $args) {  
  ...
  try {
    ...
    (nome, descricao, url, site, email, telefone, whatsapp, endereco, instagram, facebook, 
  ...
  try {
    ...
    foreach ($dados["servicos"] as $servicos) {
      if ($servicos["delete"] == "S" && isset($servicos["id_servico"]))
      {
        ...
        $resultado = $conn->query($sql);
      }else if ($servicos["delete"] == "N" && isset($servicos["id_servico"]))
      {
        $sql = " update ec_servico set nome = '". pg_escape_string($servicos["nome"]) ."', 
        descricao = '". pg_escape_string($servicos["descricao"]) ."' where id_servico = ".$servicos["id_servico"];
  ...
  try {
    ...
    foreach ($dados["destaques"] as $destaques) {
      if ($destaques["delete"] == "S" && isset($destaques["id_destaque"]))
      {
        ...
        $resultado = $conn->query($sql);
      }else if ($destaques["delete"] == "N" && isset($destaques["id_destaque"]))
      {
        $sql = " update ec_destaque set titulo = '". pg_escape_string($destaques["titulo"]) ."', 
        descricao = '". pg_escape_string($destaques["descricao"]) ."' where id_destaque = ".$destaques["id_destaque"];
